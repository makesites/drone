	...
	ssl : function(){
		...
		for(var host in config.ssl){
			...
			if( typeof site.credentials != "undefined" || typeof site.domain != "undefined"){
				// FIX: skip if credentials don't exist
				if( !fs.existsSync(site.credentials.key) || !fs.existsSync(site.credentials.cert) ) continue;
				...
				// CA chain
				// #29: Intermediate certs may work with SNICallback
				//if( site.credentials.ca ) creds.ca = fs.readFileSync( site.credentials.ca, 'utf8');
				if( site.credentials.ca ) creds.cert += '\n'+ fs.readFileSync( site.credentials.ca, 'utf8');
